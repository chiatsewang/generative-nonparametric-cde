...
class MyTest(unittest.TestCase):
    ...
    def test__box_transform(self):
        ...
        self.assertTrue(
            (
                box_transform(np.arange(0, 10, 0.1), 0, 10).round(2)
                == example_array
            ).all()
        )
        self.assertTrue(
            (
                box_transform(np.arange(0, 100, 1), 0, 100).round(2)
                == example_array
            ).all()
        )
        self.assertTrue(
            (
                box_transform(np.arange(-50, 50, 1), -50, 50).round(2)
                == example_array
            ).all()
        )
    ...
    def test__normalize(self):
        ...
        example_density = np.random.normal(
            loc=0, scale=0.1, size=example_range.shape
        )
        ...
        self.assertTrue(
            (1 - epsilon)
            < np.trapz(example_density, example_range)
            < (1 + epsilon)
        )
    ...
    def test__loss_pytorch(self):
        beta = np.repeat(
            a=np.arange(0, 10, 1).reshape(-1, 1), axis=1, repeats=10
        )
