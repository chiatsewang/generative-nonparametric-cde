...
def normalize(cde_estimates, tol=1e-6, max_iter=10000):
    ...
    if cde_estimates.ndim == 1:
        ...
        np.apply_along_axis(
            _normalize, 1, cde_estimates, tol=tol, max_iter=max_iter
        )
...
def _normalize(density, tol=1e-6, max_iter=500):
    ...
    if max_iter <= 0:
        raise ValueError(
            "max_iter needs to be positive. Currently %s" % (max_iter)
        )
...
def box_transform(z, z_min, z_max):
    ...
    if np.min(z) < z_min or np.max(z) > z_max:
        raise ValueError(
            "Passed minimum and maximum need to be outside the array range."
...
def remove_bumps(cde_estimates, delta, bin_size=0.01):
    ...
    if cde_estimates.ndim == 1:
        ...
        np.apply_along_axis(
            _remove_bumps, 1, cde_estimates, delta=delta, bin_size=bin_size
        )
