...
class cde_loss(nn.Module):
    ...
    def forward(self, beta, z_basis, shrink_factor=1.0):
        ...
        loss = (
            complexity_terms - 2 * fit_terms
        ).float().mean() / shrink_factor
        return loss
...
class cde_nll_loss(nn.Module):
    ...
    def forward(self, beta, z_basis, shrink_factor=1.0):
        fit_terms = (beta * z_basis.view(-1, 1).expand_as(beta)).sum(
            dim=1
        ) + 1.0
...
def cde_predict(
    model_output, z_min, z_max, z_grid, basis, delta=None, bin_size=0.01
):
